...
Open `skills/<skill>/rationalizations.md`. For each extracted excuse, add a row:
...
Once GREEN passes, the GREEN transcript IS the regression spec. For each tool call the subagent made (Read, Edit, Bash, Agent dispatch), drop a corresponding entry into `skills/<skill>/eval.yaml` under `trajectory_evals[].expected_sequence`. Use `target_contains` (regex/substring) — don't pin exact paths that will drift.
...
- `skills/<skill>/rationalizations.md` — the Rationalization Table that future agents actually load.
