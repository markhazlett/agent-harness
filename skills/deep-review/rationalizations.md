...
| "I drop LOW findings from the chat summary entirely, and I keep them in the disk report only as counts per dimension, not enumerated" | What you're calling "LOW" is the binary model's non-blocking layer — those findings belong enumerated under `## Worth thinking about`, not collapsed to per-dimension counts. `bin/deep-review-validate` does NOT reject counts-only (it checks matrix coverage, N/A justification, blocking-issue/suggestion pairing, and the verdict line — nothing else), so the contract here is on you, not the validator. A report that drops them is a verdict matrix, not a review. |
| "The contract says 'every finding above WARN' needs full evidence — LOW is below WARN, so the contract permits this, and I'm taking it" | There is no per-finding WARN/LOW ladder to hide under: findings are `(blocking)` vs `(non-blocking)`, and `PASS/WARN/FAIL/N/A` is a per-*dimension* verdict, not a per-finding severity gate. Every surviving non-blocking finding still needs verbatim evidence. Minimum-viable-pass is not the standard — the validator sets a floor; the skill's value is above it. |
...
| "Prose synthesis is the first thing I cut" | The chat summary can drop prose; the disk report's `## Summary` and its cross-dimension framing cannot. `bin/deep-review-validate` can't see prose quality — that's precisely why cutting it is invisible and tempting. Cross-dimension synthesis is what separates a deep review from a per-dim matrix. |
