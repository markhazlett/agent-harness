...
Contract for every `skills/<name>/SKILL.md` in this harness. Validated by `bin/test-frontmatter` and surfaced in `/harness-health`.
...
`bin/test-frontmatter` parses every `skills/*/SKILL.md` and asserts:
...
2. **Copy the template.** Start from `skills/_template-rigid/TEMPLATE.md` (the body), `rationalizations.md` (the sibling table), `eval.yaml` (the eval contract — required for rigid skills per `.claude/docs/skill-eval-spec.md`), and optionally `red-flags.md` (when the inline list outgrows ~12 bullets). Place the new skill at `skills/<name>/SKILL.md`.
3. **Fill the template.**
