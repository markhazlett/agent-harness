...
Pulls the latest agent-harness from upstream (`main`) into this project. Preserves project-specific configuration (`.claude/hooks/harness.config.sh`, `.claude/settings.json`), all local-only skills/agents/commands, and asks the user before overwriting anything they've edited.
...
- `preserved` — files the script will never overwrite no matter what (`harness.config.sh`).
...
agent-harness: <local_version> → <upstream_version>
  ...
  preserved:   harness.config.sh (always kept)
...
{
  "conflicts": {
    ".claude/skills/sync/SKILL.md": "keep-mine",
...
If `setup.sh` was updated, mention it — the user may want to re-run it to pick up new config knobs (it preserves their existing `harness.config.sh` answers as defaults… actually, re-running re-prompts every value, so suggest editing `harness.config.sh` directly for new fields rather than re-running unless they specifically want the wizard).
